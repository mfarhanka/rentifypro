...
<div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between mb-4">
    ...
    <a href="/staff/create" class="btn btn-primary mt-3 mt-lg-0">Add staff</a>
</div>
                ...
                <tbody>
                    ...
                    <?php foreach ($staffMembers as $staffMember) : ?>
                        <tr>
                            ...
                            <td class="text-right">
                                <a href="/staff/<?= esc((string) $staffMember['id']) ?>/edit" class="btn btn-sm btn-outline-primary">Edit</a>
                                <form action="/staff/<?= esc((string) $staffMember['id']) ?>/suspend" method="post" class="d-inline-block ml-2">
                                    <?= csrf_field() ?>
                                    <button type="submit" class="btn btn-sm btn-outline-warning">
                                        <?= (int) $staffMember['active'] === 1 ? 'Suspend' : 'Reactivate' ?>
                                    </button>
                                </form>
                                <form action="/staff/<?= esc((string) $staffMember['id']) ?>/delete" method="post" class="d-inline-block ml-2" onsubmit="return confirm('Remove this staff account?');">
                                    <?= csrf_field() ?>
